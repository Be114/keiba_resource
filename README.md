...
netkeiba.comまたはkeibalab.jpから競馬データを収集し、PostgreSQLに保存して、YAML設定による柔軟な分析を実行する包括的な競馬データ分析システムです。
...
### 1. データ収集機能
- netkeiba.comまたはkeibalab.jpからの競馬データ自動収集
- 2つのデータソースから選択可能（keibalab推奨）
...
│   ├── build_database.py       # データ収集スクレイピングツール（netkeiba用）
│   ├── keibalab_scraper.py     # keibalab.jp用スクレイパー
│   ├── webdriver_utils.py      # WebDriver管理ユーティリティ（netkeiba用）
│   └── run_analysis.py         # 分析エンジン
├── scrape_races.py             # データ収集統合エントリーポイント
...
指定期間の競馬データを収集します。2つのデータソースから選択できます：

#### keibalab.jp から収集（推奨）

```bash
# 2023年のデータを収集（デフォルトでkeibalabを使用）
python scrape_races.py --start-year 2023 --end-year 2023

# 明示的にkeibalabを指定
python scrape_races.py --start-year 2020 --end-year 2021 --scraper-type keibalab
```

**注意**: keibalab.jpは2014年以降のデータのみ提供しています。

#### netkeiba.com から収集（レガシー）

```bash
# netkeibaを明示的に指定
python scrape_races.py --start-year 2023 --end-year 2023 --scraper-type netkeiba
...
**注意**: 
- netkeiba.comのスクレイピングはSeleniumを使用するため、処理に時間がかかります
- keibalab.jpの方が高速で安定しています
...
1. ネットワーク接続確認
2. リクエスト間隔の調整
3. データソース（netkeiba.com/keibalab.jp）のサイト構造変更の確認
4. keibalabへの切り替えを検討（より安定的）
